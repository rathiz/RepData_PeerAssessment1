...
### Plotting Histogram of average daily steps
...
### Determination of mean and median of daily steps
...
The mean number of steps taken per day is **`r avgStepsPerDay`** and median number of steps taken each day is **`r medStepsPerDay`**.  Since median value is more than the mean value, distribution of steps taken per day is positively skewed.
...
with(dfPeriod, {plot(period, periodSteps, type='l',
                    main='Average steps taken during 5 minute periods of a day',
...
Period **`r periodMaxSteps`** has the maximum number of **`r maxSteps`** steps.  
...
There are **`r nRowsWithNA`** rows in the dataset where **steps** field  has **NA** value. Please note that, this is not the number of days with miising values but number of intrevals with missing values.
...
```
### Making Histogram of total steps
...
hist(new_dfDaily$dailySteps, breaks=25, main='Histogram of Daily Steps taken after imputing',
     xlab='Number of steps', ylab='Freguency')
```
### Computing new mean, median & determination of impact
...
The mean number of steps is **`r new_avgStepsPerDay`** and new median value is **`r new_medStepsPerDay`**. 
